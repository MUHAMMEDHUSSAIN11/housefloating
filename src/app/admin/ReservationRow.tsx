...
const ReservationRow: React.FC<ReservationRowProps> = ({ reservation, reservationID, onAction }) => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${statusStyles[status] || defaultStyle}`}>
        {statusIcons[status] || defaultIcon}
        {status}
      </span>
    );
  };
